...
setwd("C:\\Users\\Shubham\\Documents\\BA Case Study\\Regression Case Studies - Linear & Logistic\\Proactive Attrition Management-Logistic Regression Case Study")
getwd()
...
write.csv(my_num_data, file = "C:\\Users\\Shubham\\Documents\\BA Case Study\\Regression Case Studies - Linear & Logistic\\Proactive Attrition Management-Logistic Regression Case Study\\Data_means.csv")

# missing values
apply(is.na(testing[,]),2,sum)
...
## OUTLIER TREATMENT AT p1 AND p99
...
mydata$CUSTOMER[mydata$CUSTOMER>1095335.7] <- 1095335.7
...
mydata$CUSTOMER[mydata$CUSTOMER<1004986.9] <- 1004986.9
...
write.csv(TESTDATA, file = "C:\\Users\\Shubham\\Documents\\BA Case Study\\Regression Case Studies - Linear & Logistic\\Proactive Attrition Management-Logistic Regression Case Study\\TESTDATA.csv")
...
chisq.test(freq_table)
freq_table <- table(mydata$CHURN, mydata$INCMISS) #significant
chisq.test(freq_table)
freq_table <- table(mydata$CHURN, mydata$TRUCK) 
chisq.test(freq_table)
freq_table <- table(mydata$CHURN, mydata$RV) 
chisq.test(freq_table)
freq_table <- table(mydata$CHURN, mydata$RETACCPT) #significant
chisq.test(freq_table)
freq_table <- table(mydata$CHURN, mydata$CREDITCD) #significant
chisq.test(freq_table)
freq_table <- table(mydata$CHURN, mydata$REFER) #significant
chisq.test(freq_table)
freq_table <- table(mydata$CHURN, mydata$INCOME) #significant
chisq.test(freq_table)
freq_table <- table(mydata$CHURN, mydata$MCYCLE) 
chisq.test(freq_table)
freq_table <- table(mydata$CHURN, mydata$CREDITAD) #significant
chisq.test(freq_table)
freq_table <- table(mydata$CHURN, mydata$SETPRCM) #significant
chisq.test(freq_table)
...
write.csv(corrm, file = "C:\\Users\\Shubham\\Documents\\BA Case Study\\Regression Case Studies - Linear & Logistic\\Proactive Attrition Management-Logistic Regression Case Study\\corrm1.csv") 
...
fitt <- step(lm(CHURN ~ REVENUE
                ...
                +INCOME
                +CREDITAD
                +SETPRCM, data = mydata), direction = "both")


# dividing dataset in training and testing
...
fit<-glm(CHURN ~ REVENUE + root_MOU + RECCHRGE + root_OVERAGE + ROAM + 
           CHANGEM + CHANGER + DROPVCE + BLCKVCE + CUSTCARE + THREEWAY + 
           INCALLS + PEAKVCE + OPEAKVCE + CALLWAIT + MONTHS + UNIQSUBS + 
           ACTVSUBS + PHONES + root_EQPDAYS + CUSTOMER + AGE1 + AGE2 + 
           CHILDREN + CREDITB + CREDITC + CREDITDE + PRIZMRUR + PRIZMUB + 
           PRIZMTWN + REFURB + WEBCAP + MARRYNO + MAILRES + NEWCELLY + 
           INCMISS + INCOME + SETPRC + RETCALL ,data = training,
...
write.csv(coeff, "C:\\Users\\Shubham\\Documents\\BA Case Study\\Regression Case Studies - Linear & Logistic\\Proactive Attrition Management-Logistic Regression Case Study\\coeff.csv")

library(car)
asd <- as.matrix(vif(fit))
write.csv(asd, "C:\\Users\\Shubham\\Documents\\BA Case Study\\Regression Case Studies - Linear & Logistic\\Proactive Attrition Management-Logistic Regression Case Study\\vif1.csv")

#Check concordance 
source("C:\\Users\\Shubham\\Documents\\Linear & Logistic In R\\Concordance.R")
Concordance(fit)  ## concordance- 0.6308
...
fit2<-glm(CHURN ~ REVENUE + root_MOU + RECCHRGE + root_OVERAGE + ROAM + 
            CHANGEM + CHANGER + DROPVCE + BLCKVCE + CUSTCARE + THREEWAY + 
            INCALLS + PEAKVCE + OPEAKVCE + CALLWAIT + MONTHS + UNIQSUBS + 
            ACTVSUBS + PHONES + root_EQPDAYS + AGE1 + AGE2 + 
            CHILDREN + CREDITB + CREDITC + CREDITDE + PRIZMUB + REFURB + 
            WEBCAP + MARRYNO + MAILRES + NEWCELLY + INCMISS + INCOME + 
            SETPRC,data = training,
          family = binomial(logit))

summary(fit2)

source("C:\\Users\\Shubham\\Documents\\Linear & Logistic In R\\Concordance.R")
Concordance(fit2) ## concordance- 0.6225

#Multicollinierity Check using VIF
library(car)
asd2 <- as.matrix(vif(fit2))
write.csv(asd2, "C:\\Users\\Shubham\\Documents\\BA Case Study\\Regression Case Studies - Linear & Logistic\\Proactive Attrition Management-Logistic Regression Case Study\\vif2.csv")
...
write.csv(decile_summ_train,"C:\\Users\\Shubham\\Documents\\BA Case Study\\Regression Case Studies - Linear & Logistic\\Proactive Attrition Management-Logistic Regression Case Study\\fit_train_DA1.csv",row.names = F)
...
write.csv(decile_summ_test,"C:\\Users\\Shubham\\Documents\\BA Case Study\\Regression Case Studies - Linear & Logistic\\Proactive Attrition Management-Logistic Regression Case Study\\fit_test_DA1.csv",row.names = F)
